...
```
<br> Add a .password file to your device with the following code:
```bash
nano .password
```
...
<br> Finally, run the Docker image.
```bash
docker run -d -p 8787:8787 -v /Users/rebeccajenkins/BIOS611/RJENKI_BIOS611Project:/home/rstudio/RJENKI_BIOS611Project rjenki
```
<br> Once navigated to http://localhost:8787/, specify the directory:
```bash
cd ~/RJENKI_BIOS611Project
```
<br> Lastly, use the make command to generate the report:
```bash
make all
```
